...
public abstract class BaseG3dHudTest extends BaseG3dTest {
	...
	// public final static float rotationSpeed = 0.02f * 360f; // degrees per
	// second
	...
	// protected String models[] = new String[] { "car.obj", "cube.obj",
	// "scene.obj", "scene2.obj", "wheel.obj", "g3d/invaders.g3dj",
	// "g3d/head.g3db", "g3d/house.g3dj", "g3d/knight.g3dj",
	// "g3d/knight.g3db", "g3d/ship.obj", "g3d/shapes/cube_1.0x1.0.g3dj",
	// "g3d/shapes/cube_1.5x1.5.g3dj", "g3d/shapes/sphere.g3dj",
	// "g3d/shapes/teapot.g3dj", "g3d/shapes/torus.g3dj" };
	...
	// hud = new Stage(new ScalingViewport(Scaling.fit, PREF_HUDWIDTH,
	// PREF_HUDHEIGHT));
	// hudWidth = hud.getWidth();
	// hudHeight = hud.getHeight();
	// skin = new Skin(Gdx.files.internal("data/uiskin.json"));

	// final List<String> modelsList = new List<String>(skin);
	...
	// rotateCheckBox.setPosition(hudWidth - rotateCheckBox.getWidth(),
	// gridCheckBox.getHeight());
	// hud.addActor(rotateCheckBox);

	// moveCheckBox = new CheckBox("Move", skin);
	// moveCheckBox.setChecked(false);
	// moveCheckBox.setPosition(hudWidth - moveCheckBox.getWidth(),
	// rotateCheckBox.getTop());
	// hud.addActor(moveCheckBox);
	// }

	// protected CollapsableWindow addListWindow(String title, List list, float
	// x,
	// float y) {
	...
	public void render() {
		...
		// transform.rotate(Vector3.Y, rotation = (rotation + rotationSpeed
		// * Gdx.graphics.getRawDeltaTime()) % 360);
		// if (moveCheckBox.isChecked()) {
		// movement = (movement + moveSpeed * Gdx.graphics.getRawDeltaTime()) %
		// 1f;
	...
	// @Override
	// public void clicked(InputEvent event, float x, float y) {
	// if (getTapCount() == 2 && getHeight() - y <= getPadTop()
	// && y < getHeight() && x > 0 && x < getWidth())
	// toggleCollapsed();
	// }
	// });
	// }
